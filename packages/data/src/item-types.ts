...
export function canonicalizeItemTypeName(type?: string) {
  ...
  const normalized = itemTypeNameLookup.get(type.trim().toLowerCase());
  if (normalized) return normalized;

  if (type.trim().toLowerCase() === "miscellaneous") {
    return "Misc";
  }
...
export function itemTypeIdFromName(type?: string) {
  const canonical = canonicalizeItemTypeName(type);
  if (!canonical) return undefined;

  const entry = Object.entries(itemTypeNames).find(([, label]) => label === canonical);
  return entry ? Number(entry[0]) : undefined;
}

export function itemTypeNameFromId(itemType: number | null | undefined) {
  return itemTypeNameMap[itemType ?? -1] ?? "Item";
}
